# QA - Painel de Agentes e Conflitos
...
2. **Monitoramento de agentes**
   - Entre em `/conflitos` e confira se o header mostra o título correto e se o painel de agentes carrega com nome, e-mail e total de adolescentes acompanhados.
...
5. **Cadastros de agentes/conflitos via UI**
   - Navegue até `/agentes`, preencha o formulário e confirme que os novos agentes aparecem imediatamente na lista (verifique rede para o `POST /api/agentes` retornar 201).
   - Em `/conflitos`, use o formulário lateral para registrar um novo conflito entre bairros ou facções; o painel deve mostrar o conflito listado ao lado e a notificação pode ser testada com o botão “Notificar agentes”.
