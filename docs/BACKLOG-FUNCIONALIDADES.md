...
| **P1**     | Regiões/facções conflituosas e agentes | Mapear territórios críticos e técnicos de referência para alertas operacionais. | 1. Consolidar o painel `/inteligencia/conflitos` com formulários completos de bairros/facções e relatório de impacto.<br>2. Ligar os alertas preventivos ao mapa/estrutura e validar SSE após cada atualização.<br>3. Finalizar o ciclo de notificação (manual no painel interno e relatórios para inteligência). |
...
- **Novidades 08/11:** o cadastro de conflitos aceita multiplos envolvidos (gerando pares automaticamente) e o detalhe possui o botao "Notificar agentes", reutilizando a mesma rota do painel preventivo.
...
- **Novidades 08/11:** o cadastro permite selecionar varios envolvidos (gerando um grupo unico) e o detalhe traz o botao manual "Notificar agentes" usando a mesma rota da inteligencia preventiva.
...
- **Integração:** `lib/conflitos.ts` alimenta o painel preventivo, `lib/notificacoes/agente.ts` inclui nível de risco/link e `POST /api/conflitos/[id]/notificar` dispara alertas para os agentes correspondentes sempre que um conflito crítico é listado.
