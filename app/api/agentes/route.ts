...
export async function GET(request: NextRequest) {
  try {
    ...
    const agentes = await prisma.agenteProfissional.findMany({
    ...
    return NextResponse.json(
      { erro: "Erro ao listar agentes", detalhes: (error as Error).message },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existente) {
      return NextResponse.json(
        { erro: "Ja existe um agente com este email" },
        { status: 409 }
      );
    }

    const agente = await prisma.agenteProfissional.create({
      data: {
    ...
    return NextResponse.json(
      { erro: "Erro ao criar agente", detalhes: (error as Error).message },
      { status: 500 }
    );
  }
}
