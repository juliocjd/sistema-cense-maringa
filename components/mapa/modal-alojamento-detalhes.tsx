...
const SUICIDIO_MOTIVO_TAG = normalizarTexto("Monitorar risco de suicídio");
...
const normalizarNivelTexto = (valor?: string | null): NivelBadge | null => {
...
export default function ModalAlojamentoDetalhes({
  ...
  const suicidioAlertaId = alertasEspeciaisPorTipo.RISCO_SUICIDIO?.id ?? null;
  ...
  useEffect(() => {
    ...
    const carregar = async () => {
      try {
        ...
        setProtocoloSuicidioInfo(dados?.protocoloRiscoSuicidio ?? null);
      } catch {
        if (ativo) {
          setProtocoloSuicidioInfo(null);
        }
  ...
  const frontalSuicidioInfo = useMemo(() => {
    ...
    const nivel =
      normalizarNivelTexto(frontalOcupante.alertaRiscoSuicidioNivel ?? null) ??
      "BAIXO";
  ...
  const parseDetalheMensagem = (msg: string) => {
    ...
    const matchAlianca = msg.match(
      /^(ALIANÇA (?:FORTE|FRACA) \((?:Facção|Bairro)\)): (.+?) - (.+?) (da mesma facção|do mesmo bairro) que o rival - (.+)$/,
    );
    if (matchAlianca) {
      return {
  ...
  const formatarMotivoAmbiental = (motivo: string) => {
    ...
    if (match) {
      ...
      const matchRival = contexto.match(
        /conflito (?:interno|externo entre \w+) (?:com|envolvendo) (.+?)$/i,
      );
  ...
  const conflitosInternos = useMemo<Conflito[]>(() => {
    if (!ocupante) return [];
    return [...(ocupante.conflitosA ?? []), ...(ocupante.conflitosB ?? [])];
  }, [ocupante]);
  ...
  const riscosDetalhados = useMemo(() => {
    ...
    if (ocupante.alertaRiscoSuicidio) {
      ...
      const semVigilanciaLabel =
        ...
        (suicidioNivel === "ALTO" || suicidioNivel === "CRITICO")
    ...
    if (ocupante.alertaSaudeConfidencial) {
      ...
      const descricaoSaude =
        resumirDescricaoAlerta(ocupante.alertaSaudeDetalhes) ?? info.descricao;
      riscos.push({
  ...
  const motivosAmbientaisDetalhados = useMemo<
    MotivoAmbientalDetalhado[]
  >(() => {
  ...
  const alojamentosDisponiveis = useMemo(() => {
    ...
    const casaSelecionada = casas.find(
      (casa) => casa.id === transferenciaCasaId,
    );
    ...
    try {
      await onTransferir(
        ...
        transferenciaVerificacao.requer_justificativa
          ...
          : undefined,
        transferenciaMotivoObrigatorio ? transferenciaMotivo.trim() : undefined,
        transferenciaMotivoObrigatorio,
      );
      setTransferenciaCasaId("");
      setTransferenciaAlojamentoId("");
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "Erro ao transferir adolescente.";
      setTransferenciaErro(msg);
    } finally {
      setTransferindo(false);
    }
  ...
  const executarInterdicao = async (
    ...
    try {
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "Erro ao atualizar o alojamento.";
  ...
  const buscarSugestoes = async () => {
    ...
    try {
      const limite =
        transferenciaCasaId && casas.length > 0 ? Math.max(casas.length, 3) : 3;
      const response = await fetch(
  ...
  const colorClass = (() => {
    ...
    if (
      riscosDetalhados.some((r) => r.nivel === "ALTO" || r.nivel === "CRITICO")
    ) {
          ...
          <div className="max-h-[80vh] overflow-y-auto px-6 py-5 space-y-5">
            ...
            {statusInterditado && (
              <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-2 text-sm text-amber-800">
                Este alojamento esta interditado. Atualize os dados ou libere o
                leito na aba Interdicao.
              </div>
            )}
            {somenteLeitura && (
                ...
                {ocupante ? (
                      ...
                      {avaliacaoRisco && (
                        <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                          <button
                            type="button"
                            onClick={() =>
                              setMostrarBreakdownRisco(!mostrarBreakdownRisco)
                            }
                              ...
                              (motivo, index) => {
                                if (motivo.ehSuicidio) {
                                  return (
                                    <div
                                      key={`ambiental-${index}`}
                                      className="rounded-lg border border-amber-300 bg-white/60 p-2"
                                    >
                                ...
                                const info = formatarMotivoAmbiental(
                                  motivo.original,
                                );
                                ...
                                >
                                  <div className="flex items-start gap-2">
                                    ...
                                    <div className="flex-1 space-y-1">
                                      <div className="flex flex-wrap items-center gap-2">
                                        ...
                                        {risco.altaMedicaInfo && (
                                          <span
                                            ...
                                            title={
                                              risco.altaMedicaInfo.descricao ??
                                              undefined
                                            }
                                          >
                                            <CheckCircle size={10} />
                                      ...
                                      {risco.envolvidos &&
                                        risco.envolvidos.length > 0 && (
                                          <div className="space-y-1.5 mt-2">
                                            {risco.envolvidos.map(
                                              (envolvido, idx) => (
                                                <div
                                                  key={`${envolvido.id ?? envolvido.nome}-${idx}`}
                        ...
                        <ul className="mt-2 space-y-2 text-sm text-emerald-900">
                          {conflitosResolvidosLista.map((conflito) => {
                            ...
                            const adversarioNome =
                              ...
                              buscarAdolescentePorId(adversarioId)
                                ?.nomeCompleto ??
                              null;
                            return (
                              <li
                  ...
                  <div className="space-y-3">
                    <p className="text-sm text-slate-600">
                      Este alojamento esta livre. Utilize o botão abaixo para
                      iniciar uma nova alocação.
                    </p>
                    {somenteLeitura ? (
                      <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
              ...
              <section className="rounded-2xl border border-slate-200 p-4">
                ...
                {somenteLeitura ? (
                      ...
                      <div className="mt-4 space-y-3">
                        {sugestoes.map((sugestao) => (
                          ...
                          >
                            ...
                            {sugestoes && sugestao.alertas.length > 0 && (
                  ...
                  <div className="rounded-2xl border border-slate-200 p-4 text-sm text-slate-600 bg-slate-50">
                    Este alojamento esta livre. Para transferir alguem,
                    selecione um adolescente hospedado.
                  </div>
                )}
              </section>
            )}
              ...
              <section className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="grid gap-4 lg:grid-cols-[1.3fr,0.9fr]">
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        setInterdicaoJustificativa(event.target.value)
                      }
                      rows={2}
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  ...
                  <button
                    ...
                    onClick={() =>
                      executarInterdicao(
                        statusInterditado ? "LIBERAR" : "INTERDITAR",
                      )
                    }
                    disabled={
                      interdicaoLoading ||
